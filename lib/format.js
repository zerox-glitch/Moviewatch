...
export function guessSourceType(url) {
  ...
  if (/\.mpg(\?|$)/i.test(url) || /\.mpeg(\?|$)/i.test(url)) return "video/mpeg";
  if (/\.ts(\?|$)/i.test(url)) return "video/mp2t";
  return "video/mp4";
}

/** Extensions browsers can reliably play. Anything else gets a warning badge. */
const GOOD_VIDEO_RE = /\.(mp4|m4v|webm)$/i;

/** True when the filename is a format most browsers can't play (.ts, .avi, .mkv...). */
export function isRiskyVideo(name) {
  return Boolean(name) && !GOOD_VIDEO_RE.test(name);
}
